...
programsRouter.get('/', (_req, res) => {
  const rows = db.prepare('SELECT * FROM programs WHERE active = 1 ORDER BY updated_at DESC').all();
  res.json(rows.map(mapProgram));
});

// Signed-in user's enrollments (must precede '/:id').
programsRouter.get('/mine', requireAuth, (req: AuthedRequest, res) => {
  const rows = db.prepare('SELECT * FROM program_enrollments WHERE user_id = ? ORDER BY updated_at DESC').all(req.userId);
  res.json(rows.map(mapEnrollment));
});

programsRouter.post('/:id/enroll', requireAuth, (req: AuthedRequest, res: Response) => {
  const program = db.prepare('SELECT id FROM programs WHERE id = ? AND active = 1').get(req.params.id);
  if (!program) return res.status(404).json({ error: 'Program not found.' });
  const existing = db.prepare('SELECT * FROM program_enrollments WHERE user_id = ? AND program_id = ?').get(req.userId, req.params.id) as any;
  ...
  res.status(201).json(mapEnrollment(db.prepare('SELECT * FROM program_enrollments WHERE id = ?').get(id)));
});
...
programsRouter.patch('/enrollments/:id', requireAuth, (req: AuthedRequest, res: Response) => {
  const p = enrollPatch.safeParse(req.body);
  if (!p.success) return bad(res, p.error);
  const row = db.prepare('SELECT * FROM program_enrollments WHERE id = ? AND user_id = ?').get(req.params.id, req.userId) as any;
  ...
  res.json(mapEnrollment(db.prepare('SELECT * FROM program_enrollments WHERE id = ?').get(req.params.id)));
});

programsRouter.delete('/enrollments/:id', requireAuth, (req: AuthedRequest, res: Response) => {
  db.prepare('DELETE FROM program_enrollments WHERE id = ? AND user_id = ?').run(req.params.id, req.userId);
  res.json({ ok: true });
});
...
coachesRouter.get('/', (_req, res) => {
  const rows = db.prepare('SELECT * FROM coaches WHERE active = 1 ORDER BY rating DESC, reviews DESC').all();
  res.json(rows.map(mapCoach));
});

// The signed-in user's coaching relationship (must precede '/:id').
coachesRouter.get('/mine', requireAuth, (req: AuthedRequest, res) => {
  const row = db.prepare("SELECT * FROM coach_bookings WHERE user_id = ? AND status != 'ended' ORDER BY updated_at DESC LIMIT 1").get(req.userId) as any;
  res.json({ booking: row ? mapBooking(row) : null });
});

coachesRouter.post('/:id/book', requireAuth, (req: AuthedRequest, res: Response) => {
  const coach = db.prepare('SELECT id FROM coaches WHERE id = ? AND active = 1').get(req.params.id);
  ...
  db.prepare("UPDATE coach_bookings SET status='ended', updated_at=? WHERE user_id=? AND status!='ended'").run(now(), req.userId);
  const id = `bk_${makeId()}`;
  const t = now();
  db.prepare(
  ...
  res.status(201).json(mapBooking(db.prepare('SELECT * FROM coach_bookings WHERE id = ?').get(id)));
});

coachesRouter.delete('/booking/:id', requireAuth, (req: AuthedRequest, res: Response) => {
  db.prepare("UPDATE coach_bookings SET status='ended', updated_at=? WHERE id=? AND user_id=?").run(now(), req.params.id, req.userId);
  res.json({ ok: true });
});
...
companyRouter.get('/me', (req: AuthedRequest, res) => {
  const u = db.prepare('SELECT company_id FROM users WHERE id = ?').get(req.userId) as any;
  if (!u?.company_id) return res.json({ company: null });
  const c = db.prepare('SELECT * FROM companies WHERE id = ?').get(u.company_id) as any;
  res.json({ company: c ? mapCompany(c) : null });
});

companyRouter.post('/join', (req: AuthedRequest, res: Response) => {
  const code = String(req.body?.code ?? '').trim().toUpperCase();
  if (!code) return res.status(400).json({ error: 'Enter your company code.' });
  const c = db.prepare('SELECT * FROM companies WHERE UPPER(join_code) = ?').get(code) as any;
  if (!c) return res.status(404).json({ error: 'No company found for that code.' });
  const seatsUsed = (db.prepare('SELECT COUNT(*) n FROM users WHERE company_id = ?').get(c.id) as any).n;
  if (seatsUsed >= c.seats) return res.status(409).json({ error: 'This company has no seats left.' });
  db.prepare('UPDATE users SET company_id = ? WHERE id = ?').run(c.id, req.userId);
...
companyRouter.post('/leave', (req: AuthedRequest, res) => {
  db.prepare('UPDATE users SET company_id = NULL WHERE id = ?').run(req.userId);
  res.json({ ok: true });
});
...
adminPhase2Router.get('/programs', (_req, res) =>
  res.json(db.prepare('SELECT * FROM programs ORDER BY updated_at DESC').all().map(mapProgram)));

adminPhase2Router.post('/programs', (req: Request, res: Response) => {
  ...
  res.status(201).json(mapProgram(db.prepare('SELECT * FROM programs WHERE id = ?').get(id)));
});

adminPhase2Router.put('/programs/:id', (req: Request, res: Response) => {
  const p = programSchema.safeParse(req.body);
  if (!p.success) return bad(res, p.error);
  const r = db.prepare(
  ...
  res.json(mapProgram(db.prepare('SELECT * FROM programs WHERE id = ?').get(req.params.id)));
});

adminPhase2Router.delete('/programs/:id', (req: Request, res: Response) => {
  db.prepare('DELETE FROM programs WHERE id = ?').run(req.params.id);
  res.json({ ok: true });
});

// Enrollment roster for a program.
adminPhase2Router.get('/programs/:id/enrollments', (req: Request, res: Response) => {
...
adminPhase2Router.get('/coaches', (_req, res) =>
  res.json(db.prepare('SELECT * FROM coaches ORDER BY updated_at DESC').all().map(mapCoach)));

adminPhase2Router.post('/coaches', (req: Request, res: Response) => {
  ...
  res.status(201).json(mapCoach(db.prepare('SELECT * FROM coaches WHERE id = ?').get(id)));
});

adminPhase2Router.put('/coaches/:id', (req: Request, res: Response) => {
  const p = coachSchema.safeParse(req.body);
  if (!p.success) return bad(res, p.error);
  const r = db.prepare(
...
adminPhase2Router.delete('/coaches/:id', (req: Request, res: Response) => {
  db.prepare('DELETE FROM coaches WHERE id = ?').run(req.params.id);
  res.json({ ok: true });
});

// Booking roster across all coaches.
adminPhase2Router.get('/coach-bookings', (_req, res) => {
  const rows = db.prepare(
...
adminPhase2Router.get('/companies', (_req, res) => {
  const rows = db.prepare('SELECT * FROM companies ORDER BY updated_at DESC').all() as any[];
  res.json(rows.map((r) => ({
    ...mapCompany(r),
    seatsUsed: (db.prepare('SELECT COUNT(*) n FROM users WHERE company_id = ?').get(r.id) as any).n,
  })));
});

adminPhase2Router.get('/companies/:id', (req: Request, res: Response) => {
  const c = db.prepare('SELECT * FROM companies WHERE id = ?').get(req.params.id) as any;
...
adminPhase2Router.post('/companies', (req: Request, res: Response) => {
  const p = companySchema.safeParse(req.body);
  if (!p.success) return bad(res, p.error);
  const id = `co_${makeId()}`;
  let code = (p.data.joinCode || genCode()).toUpperCase();
  // Ensure uniqueness.
  while (db.prepare('SELECT id FROM companies WHERE UPPER(join_code) = ?').get(code)) code = genCode();
  const t = now();
  db.prepare(
  ...
  res.status(201).json(mapCompany(db.prepare('SELECT * FROM companies WHERE id = ?').get(id)));
});

adminPhase2Router.put('/companies/:id', (req: Request, res: Response) => {
  const p = companySchema.safeParse(req.body);
  if (!p.success) return bad(res, p.error);
  const existing = db.prepare('SELECT join_code FROM companies WHERE id = ?').get(req.params.id) as any;
  ...
  res.json(mapCompany(db.prepare('SELECT * FROM companies WHERE id = ?').get(req.params.id)));
});

adminPhase2Router.delete('/companies/:id', (req: Request, res: Response) => {
