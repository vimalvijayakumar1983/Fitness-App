...
logsRouter.get('/meals', (req: AuthedRequest, res: Response) => {
  const date = dateFilter(req);
  const rows = date
    ? db
        .prepare('SELECT * FROM meals WHERE user_id = ? AND date = ? ORDER BY logged_at DESC')
        .all(req.userId, date)
    : db
        .prepare('SELECT * FROM meals WHERE user_id = ? ORDER BY logged_at DESC LIMIT 200')
        .all(req.userId);
  res.json(
...
logsRouter.post('/meals', (req: AuthedRequest, res: Response) => {
...
logsRouter.get('/exercises', (req: AuthedRequest, res: Response) => {
  const date = dateFilter(req);
  const rows = date
    ? db
        .prepare('SELECT * FROM exercises WHERE user_id = ? AND date = ? ORDER BY logged_at DESC')
        .all(req.userId, date)
    : db
        .prepare('SELECT * FROM exercises WHERE user_id = ? ORDER BY logged_at DESC LIMIT 200')
        .all(req.userId);
  res.json(rows);
});

logsRouter.post('/exercises', (req: AuthedRequest, res: Response) => {
...
logsRouter.get('/moods', (req: AuthedRequest, res: Response) => {
  const date = dateFilter(req);
  const rows = date
    ? db
        .prepare('SELECT * FROM moods WHERE user_id = ? AND date = ? ORDER BY logged_at DESC')
        .all(req.userId, date)
    : db
        .prepare('SELECT * FROM moods WHERE user_id = ? ORDER BY logged_at DESC LIMIT 200')
        .all(req.userId);
  res.json(
...
logsRouter.post('/moods', (req: AuthedRequest, res: Response) => {
...
logsRouter.get('/sleep', (req: AuthedRequest, res: Response) => {
  const date = dateFilter(req);
  const rows = date
    ? db
        .prepare('SELECT * FROM sleep WHERE user_id = ? AND date = ? ORDER BY logged_at DESC')
        .all(req.userId, date)
    : db
        .prepare('SELECT * FROM sleep WHERE user_id = ? ORDER BY logged_at DESC LIMIT 200')
        .all(req.userId);
  res.json(rows);
});

logsRouter.post('/sleep', (req: AuthedRequest, res: Response) => {
...
logsRouter.get('/weights', (req: AuthedRequest, res: Response) => {
  const rows = db
    .prepare('SELECT * FROM weights WHERE user_id = ? ORDER BY date DESC LIMIT 365')
    .all(req.userId);
  res.json(rows);
});

logsRouter.post('/weights', (req: AuthedRequest, res: Response) => {
...
logsRouter.get('/water', (req: AuthedRequest, res: Response) => {
  const date = dateFilter(req) ?? todayISO();
  const rows = db
    .prepare('SELECT * FROM water WHERE user_id = ? AND date = ? ORDER BY logged_at DESC')
    .all(req.userId, date) as any[];
  const total = rows.reduce((sum, r) => sum + r.amount_ml, 0);
  res.json({ date, total_ml: total, entries: rows });
});

logsRouter.post('/water', (req: AuthedRequest, res: Response) => {
...
logsRouter.delete('/:kind/:id', (req: AuthedRequest, res: Response) => {
